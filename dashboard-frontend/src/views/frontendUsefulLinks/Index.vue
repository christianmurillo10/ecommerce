<template>
  <v-container fluid>
    ...
    <v-card>
      <v-card-title>
        <v-icon class="black--text">view_list</v-icon><span class="title">Frontend Useful Links</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-data-table :headers="headers" :items="frontendUsefulLinkList" :search="search" class="elevation-1">
...
import ModalForm from "@/components/modules/FrontendUsefulLinks/ModalForm";
...
export default {
  ...
  methods: {
    ...
    ...mapActions("frontendUsefulLinks", {
      ...
      deleteFrontendUsefulLinkData: "deleteData"
    }),
    ...
    deleteItem() {
      this.deleteFrontendUsefulLinkData(this.modalDelete.id)
        .then(response => {
          let obj = {
            alert: true,
            type: "success",
