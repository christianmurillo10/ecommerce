  ...
  <v-container fluid>
    ...
    <v-card>
      <v-card-title>
        ...
        <span class="title">Products Sub Sub-Categories</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-data-table
          :headers="headers"
          :items="productSubSubCategoryList"
          :search="search"
          class="elevation-1"
        >
          <template v-slot:items="props">
            ...
            <td class="text-xs-left">
              {{ props.item.productCategories.name }}
            </td>
            <td class="text-xs-left">
              {{ props.item.productSubCategories.name }}
            </td>
...
import ModalForm from "@/components/modules/ProductSubSubCategories/ModalForm";
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      this.deleteProductSubSubCategoryData(this.modalDelete.id)
        .then((response) => {
          let obj = {
            alert: true,
            type: "success",
