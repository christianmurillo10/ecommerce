<template>
  <v-container fluid>
    ...
    <v-card>
      <v-card-title>
        <v-icon class="black--text">view_list</v-icon><span class="title">Shipping Methods</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-data-table :headers="headers" :items="shippingMethodList" :search="search" class="elevation-1">
...
import ModalForm from "@/components/modules/ShippingMethods/ModalForm";
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      this.deleteShippingMethodData(this.modalDelete.id)
        .then(response => {
          let obj = {
            alert: true,
            type: "success",
