<template>
  <v-container fluid>
    ...
    <v-card>
      <v-card-title>
        <v-icon class="black--text">view_list</v-icon><span class="title">Shipping Method Rates</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
            ...
            <v-layout wrap row>
              <v-flex xs12 sm12 md12 lg12>
                <v-list-tile>
                  ...
                  <v-list-tile-content>
                    <v-list-tile-title class="body-2">{{ shippingMethodDataById.name }}</v-list-tile-title>
                  </v-list-tile-content>
                </v-list-tile>
                <v-list-tile>
                  <v-list-tile-action>
                    <v-list-tile-title class="body-2 font-weight-bold">Description:&nbsp;</v-list-tile-title>
                  </v-list-tile-action>
                  <v-list-tile-content>
                    <v-list-tile-title class="body-2">{{ shippingMethodDataById.description }}</v-list-tile-title>
                  </v-list-tile-content>
                </v-list-tile>
              </v-flex>
            </v-layout>
        ...
        <v-flex xs12 sm12 md12 lg12>
          <v-data-table :headers="headers" :items="shippingMethodRateList" :search="search" class="elevation-1">
            <template v-slot:items="props">
              <td class="text-xs-left">&#8369; {{ props.item.rate_amount }}</td>
              <td class="text-xs-left">&#8369; {{ props.item.subtotal_amount_from }}</td>
              <td class="text-xs-left">&#8369; {{ props.item.subtotal_amount_to }}</td>
...
import ModalFormRates from "./components/ModalFormRates";
...
export default {
  ...
  mounted() {
    ...
    this.getShippingMethodRateDataByShippingMethodId(this.$route.params.shippingMethodId);
  },
