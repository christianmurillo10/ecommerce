<template>
  <v-container fluid>
            ...
            <td class="justify-center layout px-0">
              <v-tooltip left>
                <template v-slot:activator="{ on }">
                  <v-icon small class="mr-2" @click="viewItemByProductId(props.item.product_id)" v-on="on">pageview</v-icon>
                </template>
                <span>View</span>
              </v-tooltip>
            </td>
    ...
    <v-dialog v-model="dialogView" scrollable persistent max-width="850px">
      <ModalViewStockReports ref="modalViewStockReports" @setDialog="setDialogView" />
    </v-dialog>
  </v-container>
</template>
