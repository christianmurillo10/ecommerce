<template>
  <v-container fluid>
    ...
    <v-card>
      <v-card-title>
        <v-icon class="black--text">view_list</v-icon><span class="title">Products Brands</span>
        <v-spacer></v-spacer>
...
import ModalForm from "@/components/modules/ProductBrands/ModalForm";
...
export default {
  ...
  computed: {
    ...mapState("productBrands", ["productBrandList"])
  },
  ...
  methods: {
    ...
    deleteItem() {
      this.deleteProductBrandData(this.modalDelete.id)
        .then(response => {
          let obj = {
            alert: true,
            type: "success",
