<template>
  <v-container fluid>
    ...
    <v-card>
      <v-card-title>
        <v-icon class="black--text">view_list</v-icon><span class="title">Banks</span>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-data-table :headers="headers" :items="bankList" :search="search" class="elevation-1">
...
import ModalForm from "@/components/modules/Banks/ModalForm";
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      this.deleteBankData(this.modalDelete.id)
        .then(response => {
          let obj = {
            alert: true,
            type: "success",
