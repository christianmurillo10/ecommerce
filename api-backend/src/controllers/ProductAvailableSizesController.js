...
module.exports = {
   ...
   * @routes POST /productAvailableSize/create
   */
  create: async (req, res) => {
    ...
    try {
      ...
      criteria = { where: { name: params.name }, include: [{ model: Model.Products, as: 'products' }, { model: Model.Users, as: 'users' }] };
      ...
      data = await Model.ProductAvailableSizes.findAll(criteria);
      if (_.isEmpty(data[0])) {
        await Model.ProductAvailableSizes.create(initialValues)
          .then(() => Model.ProductAvailableSizes.findOrCreate(criteria))
  ...
  /**
   * Update
   * @route PUT /productAvailableSize/update/:id
   * @param req
   * @param res
   * @returns {never}
   */
  update: async (req, res) => {
    ...
    try {
      ...
      data = await Model.ProductAvailableSizes.findByPk(req.params.id);
      if (!_.isEmpty(data)) {
        await data.update(initialValues)
          .then(() => Model.ProductAvailableSizes.findByPk(data.id, criteria)
          .then(finalData => {
            res.json({
              status: 200,
  ...
  /**
   * Delete
   * @route PUT /productAvailableSize/delete/:id
   * @param req
   * @param res
   * @returns {never}
   */
  ...
  /**
   * Search
   * @route POST /productAvailableSize/search/:value
   * @param req
   * @param res
   * @returns {never}
   */
  ...
  /**
   * Find all
   * @route GET /productAvailableSize
   * @param req
   * @param res
   * @returns {never}
   */
   ...
   * @route GET /productAvailableSize/findAllbyProductId/:productId
   * @param req
   * @param res
   * @returns {never}
   */
  ...
  /**
   * Find by id
   * @route GET /productAvailableSize/:id
   * @param req
   * @param res
   * @returns {never}
   */
  findById: async (req, res) => {
    ...
    try {
      ...
      criteria = { where: { is_deleted: 0 }, include: [{ model: Model.Products, as: 'products' }, { model: Model.Users, as: 'users' }] };
      // Execute findAll query
      data = await Model.ProductAvailableSizes.findByPk(req.params.id, criteria);
      if (!_.isEmpty(data)) {
        res.json({
          status: 200,
