...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      if (_.isEmpty(params.url)) return res.json({ status: 200, message: "Url is required.", result: false });
  ...
  update: async (req, res) => {
    ...
    try {
      // Pre-setting variables
      initialValues = _.pick(params, ['name', 'url']);
  ...
  /**
   * Search
   * @route POST /frontendUsefulLinks/search/:value
   * @param req
   * @param res
   * @returns {never}
   */
